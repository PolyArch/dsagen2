...
object BitwiseNot extends IsFunctionUnitImplementation {
  val supportOperation: collection.Set[Operation.DsaOperation] = Set(Operation.BitwiseNot)
  ...
  def implement(
    ...
    require(A.getWidth == dataPathBit, s"Bitwise operation should be applied to operands whose bitwidth is same")
    // Calculate the result
    val C: UInt = (~A).asUInt() // the actuall computation
    // Output
