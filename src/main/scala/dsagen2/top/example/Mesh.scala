...
class Mesh(val meshSize : Int)(implicit p: Parameters) extends DSAGen {
  /* ----- Node Creation ----- */

  // Sync Nodes
  val ivps: Array[IVPNodeModule] = Array.fill(meshSize)(createIVP(new FullIVPConfig))
  val ovps: Array[OVPNodeModule] = Array.fill(meshSize)(createOVP(new FullOVPConfig))

  // Compute Nodes
  val swMesh: Array[Array[CompNodeModule]] = createCompNodeMesh(new DefaultSWConfig, meshSize, meshSize)
  val peMesh: Array[Array[CompNodeModule]] = createCompNodeMesh(new PE_Simple64_Config, meshSize - 1, meshSize - 1)
