...
// Use fused MulAdd IP with Add operation (no operation input)
class FMulAdd_D64_IO extends Bundle {
  // Clock
  ...
  val s_axis_c_tvalid: Bool = Input(Bool())
  val s_axis_c_tready: Bool = Output(Bool())
  val s_axis_c_tdata:  UInt = Input(UInt(64.W))
  // Result
...
class FMulAdd_D64(val stage: Int = 3) extends BlackBox with HasBlackBoxResource {
  val moduleName: String = s"FMulAdd_D64_${stage}stage"

  override def desiredName: String = moduleName

  val io: FMulAdd_D64_IO = IO(new FMulAdd_D64_IO)
