...
import dsagen2.top.config.operation.Operation.FixedMax
import dsagen2.top.config.operation.{DataType, OperDataType, Operation}

import scala.collection.Set

object Max extends IsFunctionUnitImplementation {
  override val supportOperation: collection.Set[Operation.DsaOperation] = Set(FixedMax)
      ...
      case opt @ DsaOperDataType(_, dataType, _) =>
        ...
        val vecResults: Seq[UInt] = (0 until dataPathBits / unitBits).map { vecIdx =>
          ...
          if (vecIdx < vecWidth) {
            ...
            iRwire := (if (dataType.get.isSigned) (iA.asSInt().max(iB.asSInt())).asUInt() else iA.max(iB))
          } else {
            iRwire := 0.U
          }
          iRwire
