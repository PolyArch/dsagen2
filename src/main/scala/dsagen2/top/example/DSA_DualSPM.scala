...
class DSA_DualSPM(implicit p: Parameters) extends DSAGen {
  /* ----- Node Creation ----- */
  ...
  val peMesh:   Array[Array[CompNodeModule]] = createCompNodeMesh(new PE_Full64_SIMD_Config, meshSize, vpWidth - 1)

  // Memory Nodes
  val dma0: MemNodeModule = createMemNode(new DefaultDMAConfig)
  val spm0: MemNodeModule = createMemNode(new DefaultSPMConfig)
  val spm1: MemNodeModule = createMemNode(new DefaultSPMConfig)
