...
import dsagen2.top.config.operation.Operation.FixedDiv
import dsagen2.top.config.operation.{DataType, OperDataType, Operation}

import scala.collection.mutable.ListBuffer

object Div extends IsFunctionUnitImplementation {
  val supportOperation: collection.Set[Operation.DsaOperation] = Set(FixedDiv)
  ...
  def implement(
    ...
    // Assign to the result exception, for fixed point division, we should only have one bit exception: divided by zero
    exception.enumHardwareField match {
      case Some(dbz) =>
        require(dbz.getWidth == 1, s"Why the exception of fixed point divided by zero is not one bit?")
        dbz := MuxLookup(opcode, false.B, outputExcpLut).asUInt()
      case None =>
        require(requirement = false, "Fixed point division operation does not have exception?")
    }
