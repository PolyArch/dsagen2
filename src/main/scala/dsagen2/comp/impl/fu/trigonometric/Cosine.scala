...
object Cosine extends IsFunctionUnitImplementation {
  ...
  val supportOperation: collection.Set[Operation.DsaOperation] = Set(Operation.FixedCosine)
  ...
  def cosTable(inBits: Int, outBits: Int): Vec[UInt] = {
    ...
    val resSeq:   Seq[BigInt] = times.map(t => BigInt(math.round(math.cos(t) * (maxOut / 2).toLong)) * 2)
  ...
  def implement(
    ...
    val unitBits2sinTab: Map[Int, Vec[UInt]] = opDataTypeSet.unitBitsSet.map { unitBits =>
      unitBits -> cosTable(inBits, unitBits) // #input is fixed to be 8-bit, 0.3 degree as minimum difference
    ...
    val outputBitsLut: Seq[(UInt, UInt)] = opDataTypeSet.toSeq.map {
      case opt @ DsaOperDataType(operation, Some(dataType), _) =>
        // Sanity Check
        require(operation == Operation.FixedCosine, s"$this only support $supportOperation")
        // Extract parameters
